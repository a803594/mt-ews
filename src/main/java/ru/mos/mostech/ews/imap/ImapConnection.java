...
public class ImapConnection extends AbstractConnection {
    ...
    public void run() {
        ...
        try {
            ...
            sendClient("* OK [" + capabilities + "] IMAP4rev1 MT-EWS " + MosTechEws.getCurrentVersion() + " server ready");
            for (; ; ) {
                line = readClient();
                        ...
                        if ("capability".equalsIgnoreCase(command)) {
                            ...
                            if (state != State.AUTHENTICATED) {
                                ...
                                if ("lsub".equalsIgnoreCase(command) || "list".equalsIgnoreCase(command)) {
                                        ...
                                        if (tokens.hasMoreTokens()) {
                                            ...
                                            if (folderQuery.endsWith("%/%") && !"/%/%".equals(folderQuery)) {
                                                ...
                                                try {
                                                    ...
                                                    MosTechEwsTray.debug(new BundleMessage("LOG_FOLDER_ACCESS_FORBIDDEN", folderQuery));
                                                } catch (HttpNotFoundException e) {
                                                    // not found, ignore
                                                    MosTechEwsTray.debug(new BundleMessage("LOG_FOLDER_NOT_FOUND", folderQuery));
                                                } catch (HttpResponseException e) {
                                                    // other errors, ignore
                                                    MosTechEwsTray.debug(new BundleMessage("LOG_FOLDER_ACCESS_ERROR", folderQuery, e.getMessage()));
                                                }
                                                if (folder != null) {
                                                    sendClient("* " + command + " (" + folder.getFlags() + ") \"/\" \"" + encodeFolderPath(folder.folderPath) + '\"');
                                                    sendClient(commandId + " OK " + command + " completed");
                                    ...
                                    if (currentFolder != null) {
                                        MosTechEwsTray.debug(new BundleMessage("LOG_IMAP_COMMAND", command, currentFolder.folderPath));
                                        TreeMap<Long, String> previousImapFlagMap = currentFolder.getImapFlagMap();
                                        if (session.refreshFolder(currentFolder)) {
                                            handleRefresh(previousImapFlagMap, currentFolder.getImapFlagMap());
                                        }
