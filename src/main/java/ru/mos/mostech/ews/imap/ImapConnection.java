...
import ru.mos.mostech.ews.exception.HttpForbiddenException;
...
public class ImapConnection extends AbstractConnection {
	...
	public void doRun() {
		...
		try {
			...
			sendClient(
					"* OK [" + capabilities + "] IMAP4rev1 MT-EWS " + MosTechEws.getCurrentVersion() + " server ready");
			for (;;) {
				line = readClient();
							...
							if (state != State.AUTHENTICATED) {
								...
								if ("lsub".equalsIgnoreCase(command) || "list".equalsIgnoreCase(command)) {
									if (tokens.hasMoreTokens()) {
										...
										if (tokens.hasMoreTokens()) {
											...
											if (folderQuery.endsWith("%/%") && !"/%/%".equals(folderQuery)) {
												...
												boolean wildcard = folderQuery.endsWith("%")
														&& !folderQuery.contains("/") && !folderQuery.equals("%");
												boolean recursive = folderQuery.endsWith("*");
												sendSubFolders(command,
														folderQuery.substring(0, folderQuery.length() - 1), recursive,
														wildcard, specialOnly && !folderQuery.equals("%"));
												sendClient(commandId + " OK " + command + " completed");
											}
											else {
												...
												if (ranges == null) {
													...
													while (uidRangeIterator.hasNext()) {
														...
														try {
															...
															log.warn("Ignore missing message "
									...
									if (currentFolder == null) {
										...
										while (rangeIterator.hasNext()) {
											...
											try {
												...
												// client closed connection, rethrow
												// exception
												throw e;
											}
											catch (IOException e) {
									...
									if (flags != null) {
										...
										// parse flags, on create read and draft flags are
										// on the
										// same messageFlags property, 8 means draft and 1
										// means read
									...
									try {
										ExchangeSession.Message createdMessage = MessageCreateThread.createMessage(
												session, folderName, messageName, properties, mimeMessage, os,
												capabilities);
									...
									try {
										...
										// use folder.loadMessages() for small folders
										// only
										log.debug("*");
										os.write('*');
											...
											if ("UIDNEXT".equalsIgnoreCase(token)) {
												if (folder.getMessagesCount() == 0) {
													...
													if (folder.getMessagesCount() == 0) {
														...
														answer.append("UIDNEXT ")
															.append(folder.getUidNext())
															.append(' ');
													}
												}
		...
		if (spaceIndex < 0) {
			...
			if (command.equals(lastCommand)) {
				...
				if (lastCommandCount > 100 && !"NOOP".equalsIgnoreCase(lastCommand)
					...
					// more than a hundred times the same command => this is a client
					// infinite loop, close connection
			...
			while (paramTokens.hasMoreTokens()) {
				...
				if ("FLAGS".equals(param)) {
					...
					if ((parameters.contains("BODY.PEEK[HEADER.FIELDS (")
							...
							&& !parameters.contains("X-LABEL")) || parameters.equals("RFC822.SIZE RFC822.HEADER FLAGS") // icedove
							|| Settings.getBooleanProperty("mt.ews.imapAlwaysApproxMsgSize")) { // Send
																								// approximate
																								// size
						size = message.size;
						log.debug(
								String.format("Message %s sent approximate size %d bytes", message.getImapUid(), size));
					}
					else {
					...
					try {
						...
						throw new MosTechEwsException("EXCEPTION_INVALID_DATE", message.date);
					}
				}
					...
					if ((partIndexString == null || partIndexString.length() == 0) && !"RFC822.HEADER".equals(param)) {
						...
						partInputStream = messageWrapper.getRawInputStream();
							...
							|| (partIndexString != null && partIndexString.startsWith("HEADER"))) {
						// Header requested fetch headers
						String[] requestedHeaders = getRequestedHeaders(partIndexString);
						// OSX Lion special flags request
						if (requestedHeaders != null && requestedHeaders.length == 1
								&& "content-class".equals(requestedHeaders[0]) && message.contentClass != null) {
	...
	protected void handleStore(String commandId, AbstractRangeIterator rangeIterator, String action, String flags)
		...
		// auto expunge
		if (Settings.getBooleanProperty("mt.ews.imapAutoExpunge")) {
			if (expunge(false)) {
				session.refreshFolder(currentFolder);
			}
		}
	...
	private void appendEnvelope(StringBuilder buffer, MimePart mimePart)
			throws UnsupportedEncodingException, MessagingException {
		buffer.append('(');
		// Envelope for date, subject, from, sender, reply-to, to, cc, bcc,in-reply-to,
		// message-id
	...
	protected void appendBodyStructure(StringBuilder buffer, MimeMultipart multiPart)
		...
		if (slashIndex < 0) {
			throw new MosTechEwsException("EXCEPTION_INVALID_CONTENT_TYPE", multiPart.getContentType());
		}
		int semiColonIndex = multiPart.getContentType().indexOf(';');
		if (semiColonIndex < 0) {
			buffer.append(" \"")
				.append(multiPart.getContentType().substring(slashIndex + 1).toUpperCase())
				.append("\")");
		}
		else {
			buffer.append(" \"")
				.append(multiPart.getContentType().substring(slashIndex + 1, semiColonIndex).trim().toUpperCase())
				.append("\")");
		}
	}
	...
	protected void appendBodyStructure(StringBuilder buffer, MimePart bodyPart) throws IOException, MessagingException {
		...
		if (slashIndex < 0) {
			throw new MosTechEwsException("EXCEPTION_INVALID_CONTENT_TYPE", contentType);
		}
	...
	protected void sendSubFolders(String command, String folderPath, boolean recursive, boolean wildcard,
			boolean specialOnly) throws IOException {
		try {
	...
	protected ExchangeSession.Condition appendNotSearchParams(String token, SearchConditions conditions)
			throws IOException {
		ImapTokenizer innerTokens = new ImapTokenizer(token);
	...
	protected ExchangeSession.Condition appendSearchParam(ImapTokenizer tokens, String token,
		...
		if ("OLD".equals(token) || "RECENT".equals(token) || "ALL".equals(token)) {
			...
			throw new MosTechEwsException("EXCEPTION_INVALID_SEARCH_PARAMETERS", token);
		}
	...
	protected ExchangeSession.Condition appendDateSearchParam(ImapTokenizer tokens, String token) throws IOException {
		...
		try {
			...
			throw new MosTechEwsException("EXCEPTION_INVALID_SEARCH_PARAMETERS", dateToken);
		}
	...
	private static final class PartOutputStream extends FilterOutputStream {
		...
		public void write(int b) throws IOException {
			size++;
			if (((state != State.BODY && writeHeaders) || (state == State.BODY && writeBody)) && (size > startIndex)
					&& (bufferSize < maxSize)) {
				super.write(b);
				bufferSize++;
			}
			...
			while (currentIndex < value.length) {
				...
				if (currentChar == ' ' && quotes.isEmpty()) {
					...
					if (quotes.isEmpty()) {
						...
						else if (currentQuote == '"' && currentChar == '"' || currentQuote == '(' && currentChar == ')'
