    ...
    protected void handleRequest(byte[] inbuf, int offset) throws IOException {
        ...
        try {
            ...
            if (requestOperation == LDAP_REQ_BIND) {
                ...
                MosTechEwsTray.debug(new BundleMessage("LOG_LDAP_REQ_ABANDON_SEARCH", currentMessageId, abandonMessageId));
            } else {
    ...
    protected String getServiceInfo() throws UnknownHostException {
        if (serviceInfo == null) {
            serviceInfo = ("<?xml version='1.0' encoding='UTF-8'?>" +
                    ...
                    "<true/>" +
                    "<key>port</key>" +
                    "<integer>") + Settings.getProperty("mt.ews.caldavPort") + "</integer>" +
                    "</dict>" +
                    "<key>https</key>" +
                    "<dict>" +
                    "<key>disabled</key>" +
...
@Slf4j
                ...
                MosTechEwsTray.debug(new BundleMessage("LOG_LDAP_UNSUPPORTED_FILTER_ATTRIBUTE",
                        attributeName, value));
...
@Slf4j
    protected class SearchRunnable implements Runnable {
        ...
        public void run() {
            try {
                int size = 0;
                MosTechEwsTray.debug(new BundleMessage("LOG_LDAP_REQ_SEARCH", currentMessageId, dn, scope, sizeLimit, timelimit, ldapFilter.toString(), returningAttributes));
                ...
                if (scope == SCOPE_BASE_OBJECT) {
                    if (dn != null && dn.length() == 0) {
                        ...
                        if (session != null) {
                            ...
                            MosTechEwsTray.debug(new BundleMessage("LOG_LDAP_REQ_SEARCH_ANONYMOUS_ACCESS_FORBIDDEN", currentMessageId, dn));
                        }
                    } else {
                    ...
                    if (session != null) {
                        ...
                        size = persons.size();
                        MosTechEwsTray.debug(new BundleMessage("LOG_LDAP_REQ_SEARCH_FOUND_RESULTS", currentMessageId, size));
                ...
                if (size > 1 && size == sizeLimit) {
                    MosTechEwsTray.debug(new BundleMessage("LOG_LDAP_REQ_SEARCH_SIZE_LIMIT_EXCEEDED", currentMessageId));
        ...
        protected void sendPersons(int currentMessageId, String baseContext, Map<String, ExchangeSession.Contact> persons, Set<String> returningAttributes) throws IOException {
            ...
            for (ExchangeSession.Contact person : persons.values()) {
                ...
                MosTechEwsTray.debug(new BundleMessage("LOG_LDAP_REQ_SEARCH_SEND_PERSON", currentMessageId, ldapPerson.get("uid"), baseContext, ldapPerson));
