...
public final class MosTechEwsTray {
	...
	public static void init(boolean notray) {
		...
		log.debug("OS Name: " + System.getProperty("os.name") + " Java version: " + javaVersion
				+ ((arch != null) ? ' ' + arch : "") + " System tray " + (SystemTray.isSupported() ? "" : "not ")
				+ "supported " + ((currentDesktop == null) ? "" : "Current Desktop: " + currentDesktop));
		...
		if (!Settings.getBooleanProperty("mt.ews.server")) {
			if (!notray) {
				if ("Unity".equals(currentDesktop)) {
					...
					log.info(
							"Detected Gnome desktop, please follow instructions to restore normal systray or run MT-EWS in server mode");
				}
