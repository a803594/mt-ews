...
final class MosTechEwsNTLMEngineImpl implements NTLMEngine {
	...
	static final int FLAG_REQUEST_UNICODE_ENCODING = 0x00000001; // Unicode string
																	// encoding requested
	...
	static final int FLAG_REQUEST_SIGN = 0x00000010; // Requests all messages have a
														// signature attached, in
														// NEGOTIATE message.
	static final int FLAG_REQUEST_SEAL = 0x00000020; // Request key exchange for message
														// confidentiality in NEGOTIATE
														// message. MUST be used in
														// conjunction with 56BIT.
	static final int FLAG_REQUEST_LAN_MANAGER_KEY = 0x00000080; // Request Lan Manager key
																// instead of user session
																// key
	static final int FLAG_REQUEST_NTLMv1 = 0x00000200; // Request NTLMv1 security. MUST be
														// set in NEGOTIATE and CHALLENGE
														// both
	...
	static final int FLAG_REQUEST_128BIT_KEY_EXCH = 0x20000000; // Request explicit
																// 128-bit key exchange
	static final int FLAG_REQUEST_EXPLICIT_KEY_EXCH = 0x40000000; // Request explicit key
																	// exchange
	...
	static final int MSV_AV_EOL = 0x0000; // Indicates that this is the last AV_PAIR in
											// the list.
	static final int MSV_AV_NB_COMPUTER_NAME = 0x0001; // The server's NetBIOS computer
														// name.
	...
	static final int MSV_AV_FLAGS_ACCOUNT_AUTH_CONSTAINED = 0x00000001; // Indicates to
																		// the client that
																		// the account
																		// authentication
																		// is constrained.
	static final int MSV_AV_FLAGS_MIC = 0x00000002; // Indicates that the client is
													// providing message integrity in the
													// MIC field in the
	...
	private static final String TYPE_1_MESSAGE = new MosTechEwsNTLMEngineImpl.Type1Message().getResponse();

	Certificate peerServerCertificate = null;

	MosTechEwsNTLMEngineImpl() {
	}
	...
	static String getType3Message(final String user, final String password, final String host, final String domain,
		...
		return new MosTechEwsNTLMEngineImpl.Type3Message(domain, host, user, password, nonce, type2Flags, target,
				targetInformation)
			.getResponse();
	}
	...
	protected static class CipherGen {
		...
		public CipherGen(final long currentTime, final String domain, final String user, final String password,
				final byte[] challenge, final String target, final byte[] targetInformation,
				final byte[] clientChallenge, final byte[] clientChallenge2, final byte[] secondaryKey,
		...
		public byte[] getNTLMUserSessionKey() throws NTLMEngineException {
			if (ntlmUserSessionKey == null) {
				final MosTechEwsNTLMEngineImpl.MD4 md4 = new MosTechEwsNTLMEngineImpl.MD4();
	...
	private static void oddParity(final byte[] bytes) {
		for (int i = 0; i < bytes.length; i++) {
			...
			final boolean needsParity = (((b >>> 7) ^ (b >>> 6) ^ (b >>> 5) ^ (b >>> 4) ^ (b >>> 3) ^ (b >>> 2)
					^ (b >>> 1)) & 0x01) == 0;
			if (needsParity) {
				bytes[i] |= (byte) 0x01;
			}
		...
		NTLMMessage(final byte[] message, final int expectedType) throws NTLMEngineException {
			...
			if (type != expectedType) {
				throw new NTLMEngineException(
						"NTLM type " + expectedType + " message expected - instead got type " + type);
			}
	...
	static class Type2Message extends NTLMMessage {
		...
		Type2Message(final byte[] message) throws NTLMEngineException {
			...
			// Next 8 bytes, starting at offset 12: target field (2 ushort lengths, 1
			// ulong offset)
	...
	static class Type3Message extends MosTechEwsNTLMEngineImpl.NTLMMessage {
		...
		Type3Message(final long currentTime, final String domain, final String host, final String user,
			...
			this(currentTime, domain, host, user, password, nonce, type2Flags, target, targetInformation, null, null,
					null);
		}
		...
		Type3Message(final String domain, final String host, final String user, final String password,
				...
				final Certificate peerServerCertificate, final byte[] type1Message, final byte[] type2Message)
				throws NTLMEngineException {
			this(System.currentTimeMillis(), domain, host, user, password, nonce, type2Flags, target, targetInformation,
					peerServerCertificate, type1Message, type2Message);
		}
		...
		Type3Message(final long currentTime, final String domain, final String host, final String user,
			...
			final MosTechEwsNTLMEngineImpl.CipherGen gen = new MosTechEwsNTLMEngineImpl.CipherGen(currentTime,
					unqualifiedDomain, user, password, nonce, target, responseTargetInformation);
		...
		protected void buildMessage() {
			...
			if (computeMic) {
				// Computation of message integrity code (MIC) as specified in [MS-NLMP]
				// section 3.2.5.1.2.
				final MosTechEwsNTLMEngineImpl.HMACMD5 hmacMD5 = new HMACMD5(exportedSessionKey);
		...
		private byte[] addGssMicAvsToTargetInfo(final byte[] originalTargetInfo,
			...
			final int appendLength = originalTargetInfo.length - 4; // last tag is
																	// MSV_AV_EOL, do not
																	// copy that
	...
	public String generateType3Msg(final String username, final String password, final String domain,
		...
		// need to retrieve raw type 1 and 2 message and connection certificate for
		...
		log.debug("generateType3Msg type2Flags " + t2m.getFlags() + " target='" + t2m.getTarget() + " username='"
				+ username + "'");

		return getType3Message(username, password, workstation, domain, t2m.getChallenge(), t2m.getFlags(),
				t2m.getTarget(), t2m.getTargetInfo(), peerServerCertificate, type1MessageBytes, type2MessageBytes);
	}
