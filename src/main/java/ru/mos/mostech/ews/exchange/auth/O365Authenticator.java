...
import ru.mos.mostech.ews.ui.NumberMatchingFrame;
...
public class O365Authenticator implements ExchangeAuthenticator {
	...
	public void authenticate() throws IOException {
		...
		String clientId = Settings.getProperty("mt.ews.oauth.clientId", "facd6cff-a294-4415-b59f-c5b01937d7bd");
		// standard native app redirectUri
		String redirectUri = Settings.getProperty("mt.ews.oauth.redirectUri",
				Settings.O365_LOGIN_URL + "common/oauth2/nativeclient");
		// company tenantId or common
		tenantId = Settings.getProperty("mt.ews.oauth.tenantId", "common");
		...
		try (HttpClientAdapter httpClientAdapter = new HttpClientAdapter(url, userid, password)) {
			...
			if (!responseBodyAsString.contains("Config=")) {
				...
				RestRequest getCredentialMethod = new RestRequest(
						Settings.getO365LoginUrl() + tenantId + "/GetCredentialType");
				...
				if (federationRedirectUrl != null && !federationRedirectUrl.isEmpty()) {
					...
					logonMethod.setRequestHeader("Accept",
					...
					if (location == null || !location.toString().startsWith(redirectUri)) {
						...
						if (config.optJSONArray("arrScopes") != null
								|| config.optJSONArray("urlPostRedirect") != null) {
							log.warn(
									"Authentication successful but user consent or validation needed, please open the following url in a browser");
							log.warn(url);
							...
							throw new MosTechEwsAuthenticationException("LOG_MESSAGE",
									"Authentication failed, unknown error: " + config);
						}
					}
					...
					if (query.contains("code=")) {
						...
						throw new MosTechEwsAuthenticationException("LOG_MESSAGE",
								"Authentication failed, unknown error: " + query);
					}
				}
			}
	...
	private String authenticateRedirectADFS(HttpClientAdapter httpClientAdapter, String federationRedirectUrl,
			String authorizeUrl) throws IOException, JSONException {
		// get ADFS login form
	...
	private String authenticateADFS(HttpClientAdapter httpClientAdapter, String responseBodyAsString,
			String authorizeUrl) throws IOException, JSONException {
		URI location;
		...
		if (responseBodyAsString.contains("login.microsoftonline.com")) {
			...
			PostRequest logonMethod = new PostRequest(
					extract("method=\"post\" action=\"([^\"]+)\"", responseBodyAsString));
			logonMethod.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		...
		if (query == null) {
			// failover for null query with non https URI like
	...
	private URI processDeviceLogin(HttpClientAdapter httpClient, URI location) throws IOException, JSONException {
		URI result = location;
		log.debug(
				"Proceed to device authentication, must have access to a client certificate signed by MS-Organization-Access");
		if (Settings.isWindows() && (System.getProperty("java.version").compareTo("13") < 0
				|| !"MSCAPI".equals(Settings.getProperty("mt.ews.ssl.clientKeystoreType")))) {
			log.warn(
					"MSCAPI and Java version 13 or higher required to access TPM protected client certificate on Windows");
		}
	...
	private URI handleMfa(HttpClientAdapter httpClientAdapter, PostRequest logonMethod, String username,
			String clientRequestId) throws IOException, JSONException {
	...
	private String executeFollowRedirect(HttpClientAdapter httpClientAdapter, GetRequest getRequest)
			throws IOException {
		...
		if (responseWrapper.getURI().getHost().endsWith("okta.com")) {
			throw new MosTechEwsAuthenticationException("LOG_MESSAGE",
					"Okta authentication not supported, please try O365Interactive");
		}
