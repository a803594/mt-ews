...
public class O365Token {
	...
	public O365Token(String tenantId, String clientId, String redirectUri, String code, String password)
			throws IOException {
		this.clientId = clientId;
		this.redirectUri = redirectUri;
		this.tokenUrl = Settings.getO365LoginUrl() + tenantId + "/oauth2/token";
	...
	public void setJsonToken(JSONObject jsonToken) throws IOException {
		try {
			...
			log.debug("Access token expires " + new Date(expiresOn));

			// get username from id_token
			String idToken = jsonToken.optString("id_token");
			if (idToken != null && idToken.contains(".")) {
				String decodedJwt = IOUtil
					.decodeBase64AsString(idToken.substring(idToken.indexOf("."), idToken.lastIndexOf(".")));
				try {
					JSONObject tokenBody = new JSONObject(decodedJwt);
					log.debug("Token: " + tokenBody);
					username = tokenBody.getString("unique_name");
				}
				catch (JSONException e) {
					log.warn("Invalid id_token " + e.getMessage(), e);
				}
			}
			// failover: get username from bearer
			if (username == null) {
				String decodedBearer = IOUtil.decodeBase64AsString(
	...
	private void executeRequest(RestRequest tokenMethod) throws IOException {
		...
		try (HttpClientAdapter httpClientAdapter = new HttpClientAdapter(tokenUrl);
				CloseableHttpResponse response = httpClientAdapter.execute(tokenMethod)) {
			setJsonToken(tokenMethod.handleResponse(response));
		}
	}

	static O365Token build(String tenantId, String clientId, String redirectUri, String code, String password)
			throws IOException {
		...
		if (Settings.getBooleanProperty("mt.ews.oauth.persistToken", true)) {
			...
			if (encryptedRefreshToken != null) {
				...
				try {
					...
					log.debug("Loaded stored token for " + username);
					...
					log.debug("Authenticated user " + localToken.getUsername() + " from stored token");
					token = localToken;
