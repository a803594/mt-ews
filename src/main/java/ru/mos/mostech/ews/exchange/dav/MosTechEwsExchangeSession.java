...
@Slf4j
    ...
    public MosTechEwsExchangeSession(HttpClientAdapter httpClientAdapter, java.net.URI uri, String userName) throws IOException {
    ...
    public class Event extends ExchangeSession.Event {
        ...
        private byte[] getICSFromItemProperties() throws HttpNotFoundException {
            ...
            try {
                ...
                MultiStatusResponse[] responses = searchItems(folderPath, eventProperties, MosTechEwsExchangeSession.this.isEqualTo("urlcompname", convertItemNameToEML(itemName)), FolderQueryTraversal.Shallow, 1);
                if (responses.length == 0) {
...
@Slf4j
                ...
                // trigger activeSync push event, only if mt-ews.forceActiveSyncUpdate setting is true
                if ((status == HttpStatus.SC_OK || status == HttpStatus.SC_CREATED) &&
            ...
            try {
                ...
                for (ExchangeSession.Event event : events) {
                    if (itemName.equals(event.getName())) {
                        HttpPropfind permanentHttpPropfind = new HttpPropfind(encodeAndFixUrl(((MosTechEwsExchangeSession.Event) event).getPermanentUrl()), EVENT_REQUEST_PROPERTIES_NAME_SET, 0);
...
@Slf4j
    ...
    protected String getTimezoneIdFromExchange() {
        ...
        try {
            ...
            MultiStatusResponse[] responses = searchItems("/users/" + getEmail() + "/NON_IPM_SUBTREE", attributes, isEqualTo("messageclass", "IPM.Configuration.OWA.UserOptions"), MosTechEwsExchangeSession.FolderQueryTraversal.Deep, 1);
            if (responses.length == 1) {
    ...
    public Message createMessage(String folderPath, String messageName, HashMap<String, String> properties, MimeMessage mimeMessage) throws IOException {
        ...
        if (!davProperties.isEmpty()) {
            ...
            try (CloseableHttpResponse response = httpClientAdapter.execute(httpProppatch)) {
                ...
                int statusCode = response.getStatusLine().getStatusCode();
                if (statusCode != HttpStatus.SC_MULTI_STATUS) {
                    throw new MosTechEwsException("EXCEPTION_UNABLE_TO_CREATE_MESSAGE", messageUrl, statusCode, ' ', response.getStatusLine().getReasonPhrase());
                }
        ...
        try {
            ...
            if (code != HttpStatus.SC_OK && code != HttpStatus.SC_CREATED) {
                ...
                if (code == HttpStatus.SC_INSUFFICIENT_STORAGE) {
                    ...
                    throw new MosTechEwsException("EXCEPTION_UNABLE_TO_CREATE_MESSAGE", messageUrl, code, ' ', reasonPhrase);
                }
            }
            ...
            inputStream = new FilterInputStream(inputStream) {
                ...
                public int read(byte[] buffer, int offset, int length) throws IOException {
                    ...
                    if (totalCount - lastLogCount > 1024 * 128) {
                        MosTechEwsTray.debug(new BundleMessage("LOG_DOWNLOAD_PROGRESS", String.valueOf(totalCount / 1024), httpGet.getURI()));
