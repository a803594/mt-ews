...
public class WebdavExchangeSession extends ExchangeSession {
	...
	protected void getEmailAndAliasFromOptions() {
		...
		try (CloseableHttpResponse response = httpClientAdapter.execute(optionsMethod, cloneContext());
			...
			while ((line = optionsPageReader.readLine()) != null && (line.indexOf('[') == -1 || line.indexOf('@') == -1
					|| line.indexOf(']') == -1 || !line.toLowerCase().contains(MAILBOX_BASE))) {
			}
			if (line != null) {
		...
		if (folderPath.startsWith(INBOX)) {
			...
			if (principalIndex >= 0) {
				...
				if (localPath.startsWith(LOWER_CASE_INBOX) || localPath.startsWith(INBOX)
						|| localPath.startsWith(MIXED_CASE_INBOX)) {
	...
	public void galLookup(Contact contact) {
		if (!disableGalLookup) {
			...
			HttpGet httpGet = new HttpGet(
					URIUtil.encodePathQuery(getCmdBasePath() + "?Cmd=gallookup&ADDR=" + contact.get("smtpemail1")));
			try (CloseableHttpResponse response = httpClientAdapter.execute(httpGet)) {
				Map<String, Map<String, String>> results = XMLStreamUtil
					.getElementContentsAsMap(response.getEntity().getContent(), "person", "alias");
	...
	protected String getFreeBusyData(String attendee, String start, String end, int interval) throws IOException {
		...
		String fbdata;
		try (CloseableHttpResponse response = httpClientAdapter.execute(httpGet)) {
			fbdata = StringUtil.getLastToken(new BasicResponseHandler().handleResponse(response), "<a:fbdata>",
					"</a:fbdata>");
		}
		return fbdata;
	}
	...
	@Override
	public void buildSessionInfo(java.net.URI uri) throws MosTechEwsException {
		buildMailPath(uri);
	...
	protected String getMailpathFromWelcomePage(java.net.URI uri) {
		...
		try (CloseableHttpResponse response = httpClientAdapter.execute(method);
				InputStream inputStream = response.getEntity().getContent();
				BufferedReader mainPageReader = new BufferedReader(
						new InputStreamReader(inputStream, StandardCharsets.UTF_8))) {
			String line;
			// noinspection StatementWithEmptyBody
	...
	protected void buildMailPath(java.net.URI uri) throws MosTechEwsAuthenticationException {
		...
		log.debug("Current user email is " + email + ", alias is " + alias + ", mailPath is " + mailPath + " on "
				+ serverVersion);
	...
	public void buildEmail(String hostName) {
		...
		if (mailBoxPath != null && mailBoxPath.indexOf('@') >= 0) {
			...
			if (email == null) {
				log.debug("Unable to get user email with alias " + mailBoxPath + " or " + getAliasFromLogin() + " or "
						+ alias);
	...
	protected void fixClientHost(java.net.URI currentUri) {
		// update client host, workaround for Exchange 2003 mailbox with an Exchange 2007
		// frontend
	...
	protected void getWellKnownFolders() throws MosTechEwsException {
		...
		try {
			...
			log.debug("Inbox URL: " + inboxUrl + " Trash URL: " + deleteditemsUrl + " Sent URL: " + sentitemsUrl
	...
	public class Contact extends ExchangeSession.Contact {
		...
		protected ExchangePropPatchRequest internalCreateOrUpdate(String encodedHref) throws IOException {
			...
			try (CloseableHttpResponse response = httpClientAdapter.execute(propPatchRequest)) {
				log.debug("internalCreateOrUpdate returned " + response.getStatusLine().getStatusCode() + " "
			...
			if (status == HttpStatus.SC_MULTI_STATUS) {
				...
				if (responses.length == 1) {
					...
					if (status == HttpStatus.SC_MULTI_STATUS) {
						...
						log.warn("Unable to create or update contact " + status + ' '
								+ propPatchRequest.getStatusLine());
					}
				}
				...
				log.warn("Unable to create or update contact " + status + ' '
						+ propPatchRequest.getStatusLine().getReasonPhrase());
			}
			...
			if (status == HttpStatus.SC_OK || status == HttpStatus.SC_CREATED) {
				...
				if (photo != null) {
					...
					try (CloseableHttpResponse response = httpClientAdapter.execute(attachmentPropPatchRequest)) {
						...
						status = response.getStatusLine().getStatusCode();
						if (status != HttpStatus.SC_MULTI_STATUS) {
							log.error("Error in contact photo create or update: "
	...
	public class Event extends ExchangeSession.Event {
		...
		public byte[] getEventContent() throws IOException {
			byte[] result = null;
			log.debug("Get event subject: " + subject + " contentclass: " + contentClass + " href: " + getHref()
		...
		private byte[] getICSFromItemProperties() throws HttpNotFoundException {
			...
			try {
				...
				localVCalendar.setPropertyValue("PRODID", "-//mos.ru/NONSGML MT-EWS Calendar V1.1//EN");
				...
				if (instancetype == null) {
					...
					vEvent.setPropertyValue("COMPLETED;VALUE=DATE",
							convertDateFromExchangeToTaskDate(getPropertyIfExists(davPropertySet, "datecompleted")));
					...
					if (dtstart != null) {
						...
						log.warn(
								"missing dtstart on item, using fake value. Set mt.ews.deleteBroken=true to delete broken events");
					...
					if (dtend != null) {
						...
						log.warn(
								"missing dtend on item, using fake value. Set mt.ews.deleteBroken=true to delete broken events");
				...
				throw new HttpNotFoundException("Unable to get event " + getName() + " subject: " + subject + " at "
						+ permanentUrl + ": " + e.getMessage());
			}
		...
		protected CloseableHttpResponse internalCreateOrUpdate(String encodedHref, byte[] mimeContent)
				throws IOException {
		...
		public ItemResult createOrUpdate() throws IOException {
			...
			if (vCalendar.isTodo()) {
				...
				if (noneMatch != null) {
					...
					propertyValues
						.add(Field.createPropertyValue("calendaruid", vCalendar.getFirstVeventPropertyValue("UID")));
				}
				propertyValues
					.add(Field.createPropertyValue("subject", vCalendar.getFirstVeventPropertyValue("SUMMARY")));
				propertyValues.add(
						Field.createPropertyValue("description", vCalendar.getFirstVeventPropertyValue("DESCRIPTION")));
				...
				propertyValues
					.add(Field.createPropertyValue("keywords", vCalendar.getFirstVeventPropertyValue("CATEGORIES")));
				...
				if (status == HttpStatus.SC_OK) {
					...
					if (responses.length == 1) {
						...
						if (status == HttpStatus.SC_OK) {
							...
							log.warn("Unable to create or update event " + status + ' '
				...
				// trigger activeSync push event, only if mt-ews.forceActiveSyncUpdate
				// setting is true
				if ((status == HttpStatus.SC_OK || status == HttpStatus.SC_CREATED)
					...
					propertyList
						.add(Field.createDavProperty("internetContent", IOUtil.encodeBase64AsString(mimeContent)));
	...
	protected Folder internalGetFolder(String folderPath) throws IOException {
		MultiStatus multiStatus = httpClientAdapter.executeDavRequest(
				new HttpPropfind(URIUtil.encodePath(getFolderPath(folderPath)), FOLDER_PROPERTIES_NAME_SET, 0));
		MultiStatusResponse[] responses = multiStatus.getResponses();
	...
	public int createFolder(String folderPath, String folderClass, Map<String, String> properties) throws IOException {
		...
		// ExchangePropPatchRequest instead
		ExchangePropPatchRequest propPatchRequest = new ExchangePropPatchRequest(
				URIUtil.encodePath(getFolderPath(folderPath)), propertyValues) {
			@Override
	...
	protected void moveItem(HttpMove httpMove) throws IOException {
		try (CloseableHttpResponse response = httpClientAdapter.execute(httpMove)) {
			int statusCode = response.getStatusLine().getStatusCode();
			if (statusCode == HttpStatus.SC_PRECONDITION_FAILED) {
				throw new MosTechEwsException("EXCEPTION_UNABLE_TO_MOVE_ITEM");
			}
	...
	protected String getURLPropertyIfExists(DavPropertySet properties,
			@SuppressWarnings("SameParameterValue") String alias) throws IOException {
	...
	protected long getLongPropertyIfExists(DavPropertySet properties,
			@SuppressWarnings("SameParameterValue") String alias) {
		DavProperty property = properties.get(Field.getPropertyName(alias));
		if (property == null) {
			return 0;
		}
	...
	protected double getDoublePropertyIfExists(DavPropertySet properties,
			@SuppressWarnings("SameParameterValue") String alias) {
		DavProperty property = properties.get(Field.getResponsePropertyName(alias));
		if (property == null) {
			return 0;
	...
	protected byte[] getBinaryPropertyIfExists(DavPropertySet properties,
			@SuppressWarnings("SameParameterValue") String alias) {
		byte[] property = null;
	...
	public MessageList searchMessages(String folderPath, Set<String> attributes, Condition condition)
		...
		MultiStatusResponse[] responses = searchItems(folderPath, attributes,
				and(isFalse("isfolder"), isFalse("ishidden"), condition), FolderQueryTraversal.Shallow, maxCount);
	...
	protected Condition getCalendarItemCondition(Condition dateCondition) {
		boolean caldavEnableLegacyTasks = Settings.getBooleanProperty("mt.ews.caldavEnableLegacyTasks", false);
		if (caldavEnableLegacyTasks) {
			...
			return and(
					or(isEqualTo("outlookmessageclass", "IPM.Appointment"),
							isEqualTo("outlookmessageclass", "IPM.Appointment.MeetingEvent")),
	...
	protected MultiStatusResponse[] searchItems(String folderPath, Set<String> attributes, Condition condition,
		...
		searchRequest.append(" FROM SCOPE('")
			.append(folderQueryTraversal)
			.append(" TRAVERSAL OF \"")
			.append(folderUrl)
			.append("\"')");
		if (condition != null) {
	...
	public Item getItem(String folderPath, String itemName) throws IOException {
		...
		try {
			HttpPropfind httpPropfind = new HttpPropfind(URIUtil.encodePath(itemPath),
			...
			if (responses == null || responses.length == 0 && isMainCalendar(folderPath)) {
				...
				HttpPropfind taskHttpPropfind = new HttpPropfind(
						URIUtil.encodePath(getFolderPath(TASKS) + '/' + emlItemName), EVENT_REQUEST_PROPERTIES_NAME_SET,
						0);
			...
			try {
				...
				log.warn("search by urlcompname failed, actual value is "
	...
	public int sendEvent(String icsBody) throws IOException {
		...
		byte[] mimeContent = (new Event(getFolderPath(DRAFTS), itemName, "urn:content-classes:calendarmessage", icsBody,
				null, null))
			.createMimeContent();
		if (mimeContent == null) {
	...
	protected void loadVtimezone() {
		try {
			...
			String folderPath = getFolderPath("mt-ews-temp");
			...
			if ("Exchange2003".equals(serverVersion)) {
				...
				try (CloseableHttpResponse response = httpClientAdapter.execute(httpPost)) {
					...
					if (statusCode == HttpStatus.SC_OK) {
						fakeEventUrl = StringUtil.getToken(new BasicResponseHandler().handleResponse(response),
			...
			if (fakeEventUrl == null) {
				...
				String timezoneId = Settings.getProperty("mt.ews.timezoneId");
				if (timezoneId == null) {
			...
			if (fakeEventUrl != null) {
				...
				try (CloseableHttpResponse response = httpClientAdapter.execute(httpGet)) {
					this.vTimezone = new VObject(
							"BEGIN:VTIMEZONE" + StringUtil.getToken(new BasicResponseHandler().handleResponse(response),
	...
	public Message createMessage(String folderPath, String messageName, Map<String, String> properties,
		...
		if (properties != null && properties.containsKey("draft")) {
			// note: draft is readonly after create, create the message first with
			// requested messageFlags
		...
		if (!davProperties.isEmpty()) {
			...
			try (CloseableHttpResponse response = httpClientAdapter.execute(httpProppatch)) {
				...
				int statusCode = response.getStatusLine().getStatusCode();
				if (statusCode != HttpStatus.SC_MULTI_STATUS) {
					throw new MosTechEwsException("EXCEPTION_UNABLE_TO_CREATE_MESSAGE", messageUrl, statusCode, ' ',
							response.getStatusLine().getReasonPhrase());
				}
		...
		try {
			...
			if (code != HttpStatus.SC_OK && code != HttpStatus.SC_CREATED) {
				...
				if (code == HttpStatus.SC_INSUFFICIENT_STORAGE) {
					...
					throw new MosTechEwsException("EXCEPTION_UNABLE_TO_CREATE_MESSAGE", messageUrl, code, ' ',
							reasonPhrase);
				}
			}
		}
	...
	public void updateMessage(ExchangeSession.Message message, Map<String, String> properties) throws IOException {
		...
		try (CloseableHttpResponse response = httpClientAdapter.execute(patchMethod)) {
			int statusCode = response.getStatusLine().getStatusCode();
			if (statusCode != HttpStatus.SC_MULTI_STATUS) {
				throw new MosTechEwsException("EXCEPTION_UNABLE_TO_UPDATE_MESSAGE");
			}
		}
	}
		...
		try {
			try {
				try {
					...
					// failover for misconfigured Exchange server, replace host name in
					// url
			...
			log.warn("Broken message at: " + message.messageUrl + " permanentUrl: " + message.permanentUrl
			...
			try {
				...
				HttpPropfind httpPropfind = new HttpPropfind(encodeAndFixUrl(message.permanentUrl), messageProperties,
						0);
				try (CloseableHttpResponse response = httpClientAdapter.execute(httpPropfind)) {
			...
			if (baos.size() == 0 && Settings.getBooleanProperty("mt.ews.deleteBroken")) {
				log.warn(
						"Deleting broken message at: " + message.messageUrl + " permanentUrl: " + message.permanentUrl);
				try {
					message.delete();
				}
	...
	protected String encodeAndFixUrl(String url) throws IOException {
		...
		// sometimes permanenturis inside items are wrong after an Exchange version
		// migration
		// need to restore base uri to actual public Exchange uri
		if (restoreHostName && fixedurl.startsWith("http")) {
			try {
				return URIUtils.rewriteURI(new java.net.URI(fixedurl), URIUtils.extractHost(httpClientAdapter.getUri()))
					.toString();
			}
			catch (URISyntaxException e) {
				throw new IOException(e.getMessage(), e);
	...
	protected void moveMessage(String sourceUrl, String targetFolder) throws IOException {
		...
		try (CloseableHttpResponse response = httpClientAdapter.execute(method)) {
			int statusCode = response.getStatusLine().getStatusCode();
			if (statusCode == HttpStatus.SC_PRECONDITION_FAILED || statusCode == HttpStatus.SC_CONFLICT) {
				throw new MosTechEwsException("EXCEPTION_UNABLE_TO_MOVE_MESSAGE");
			}
			else if (statusCode != HttpStatus.SC_CREATED) {
	...
	protected void copyMessage(String sourceUrl, String targetFolder) throws IOException {
		...
		try (CloseableHttpResponse response = httpClientAdapter.execute(httpCopy)) {
			int statusCode = response.getStatusLine().getStatusCode();
			if (statusCode == HttpStatus.SC_PRECONDITION_FAILED) {
				throw new MosTechEwsException("EXCEPTION_UNABLE_TO_COPY_MESSAGE");
			}
			else if (statusCode != HttpStatus.SC_CREATED) {
